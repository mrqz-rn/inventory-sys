...
const EditItemModal: React.FC<EditItemModalProps> = ({
  ...
  const validate = (): boolean => {
    ...
    if (!form.rawQty.trim() || isNaN(qty) || qty < 1) errs.quantity = 'Enter a valid quantity (min 1).';
    ...
    if (form.freight < 0 || isNaN(form.freight))    errs.freight     = 'Freight must be a valid integer (min 0).';
    if (form.duties  < 0 || isNaN(form.duties))     errs.duties      = 'Duties must be a valid integer (min 0).';
    if (form.taxes   < 0 || isNaN(form.taxes))      errs.taxes       = 'Taxes must be a valid integer (min 0).';
    ...
    >
      ...
      >
        ...
        <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex items-center justify-between gap-4">
          <div>
            <p className="text-[12px] font-black uppercase tracking-[0.22em] text-slate-400">Editing SKU</p>
            <h2 className="text-lg font-black text-slate-900 leading-snug tracking-tight">{item.name}</h2>
          </div>
          <button
            type="button"
            onClick={handleClose}
