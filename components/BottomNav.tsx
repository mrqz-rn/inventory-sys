      ...
      {tabs.map((tab) => {
        ...
        if (tab.special) {
          return (
            ...
            >
              <div className="bg-blue-600 p-4 rounded-2xl shadow-lg shadow-blue-300 active:scale-90 transition-transform text-white border-4 border-white">
                <tab.icon className="w-6 h-6" />
              </div>
              <span className="text-[10px] font-bold mt-1 text-blue-600">{tab.label}</span>
            </button>
          );
        }
        ...
        return (
          ...
          >
            <tab.icon className={`w-5 h-5 ${isActive ? 'fill-blue-50' : ''}`} />
