...
import {
  X, Calculator, Plus, Package, Truck,
  ShieldAlert, ReceiptText, MapPin, Layers, Hash, AlertCircle
} from 'lucide-react';
...
// ── Cost input card ───────────────────────────────────────────────────────────
const CostField = ({
  label, icon: Icon, accent, value, onChange, error, min
}: {
              ...
              <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    type="button"
                    onClick={handleClose}
                    className="px-5 py-3 rounded-xl text-sm font-bold text-white/70 bg-white/10 border border-white/20 hover:bg-white/20 transition-all"
                  >
                    Discard
                  </button>
                  <button
                    type="submit"
                    className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-black text-blue-700 bg-white hover:bg-blue-50 shadow-lg transition-all active:scale-95"
                  >
                    <Plus size={14} /> Register Stock
                  </button>
                </div>
              </div>
            </div>
